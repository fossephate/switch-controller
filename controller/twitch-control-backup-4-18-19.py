...
class Client(object):
	...
	def on_controller_state(data):
		...
		if (cNum > 0 and client.currentGame in singlePlayerGames and cNum < 8):
			return
		if (cNum > 1 and client.currentGame in twoPlayerGames and cNum < 8):
			return
	...
	def handleChat(self, username, message, source="twitch", uniqueID=None):
		...
		if (len(commands) >= 2):
			...
			if (commands[0] == "!goto" or (commands[0] == "!forcegoto" and username in modlist) or (commands[0] == "!setgame" and username in modlist)):
				...
				if ((game not in gameList) or len(commands) > 2):
