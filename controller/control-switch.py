...
try:
	w_handle = pywinauto.findwindows.find_windows(title="scriptswitch")[1]
	print("found")
	sys.exit()
except IndexError:
	print("not found")
...
# set window title:
ctypes.windll.kernel32.SetConsoleTitleW("scriptswitch")
...
sio = socketio.Client()
try:
	sio.connect("http://remotegames.io:8050")
except:
...
controllers = []
# for i in range(0, 8):
# 	controllers.append(SwitchController3())
...
for i in range(0, 8):
	com = "COM" + str(i + 1)
	tempController = SwitchController3()
	res = tempController.connect(com)
	if (res == "success"):
...
try:
	...
	controllers[8].connect(1)
except:
	print("controllers[8] error")
...
xboxGameList1 = ["Apex Legends", "Battleblock Theater", "Borderlands", "Braid", "Call of Duty(R): Black Ops 4", "Call of Duty(R): WaW", "Castle Crashers", "Celeste", "Crackdown", "DOOM", "Fallout 3", "Forza Horizon 4", "Grand Theft Auto V", "GTA IV",
"Halo 5: Guardians", "Halo: Reach", "Halo: The Master Chief Collection", "Hue", "LEGO Star Wars: TCS", "LEGO(C) Indiana Jones(TM)",
"Minecraft", "Modern Warfare(R) 3", "Overwatch: Origins Edition", "Q.U.B.E. 2", "Red Dead Redemption 2", "Rocket League(R)",
"Sea of Thieves", "Super Lucky's Tale", "Terraria"]
xboxGameList2 = ["apex", "battleblock", "borderlands", "braid", "bo4", "waw", "castle", "celeste", "crackdown",
"doom", "fallout3", "forzah4", "gta5", "gta4", "halo5", "halor", "halot", "hue", "legostar", "legojones",
"minecraft", "mw3", "overwatch", "qube2", "rdr2", "rocketleague", "thieves", "lucky", "terraria"]
# plusCommands = []
# modCommands = []
# anyCommands = []
...
pluslist = []
modlist = ["melodiousmarci", "harmjan387", "beanjr_yt", "alua2020", "splatax", "silvermagpi", "remotegames", "fossephate", "tpnsbot"]
banlist = []
sublist = []
voted = []
...
class Client(object):
	...
	def on_controller_state(data):
		...
		if (cNum > 0 and client.currentSwitchGame in singlePlayerGames and cNum < 8):
			return
		if (cNum > 1 and client.currentSwitchGame in twoPlayerGames and cNum < 8):
			return
	...
	def goto_switch_game(self, game, delay=50):
		...
		width = 1920
		height = 1080
		windowName = switchTitle
		iconLoc = None

		index = switchGameList1.index(game)
		timesToMoveDown = int(index / 6)
		timesToMoveRight = (index % 6)
	...
	def end_switch_goto_vote(self, game, delay):
		...
		if (leaving):
			self.goto_switch_game(game, delay)
		return

	def reenable_goto(self):
		self.gotoUsed = False

	def goto_switch_game_vote(self, game, delay=50):
		...
		self.yeaVotes = 0
		self.nayVotes = 0
		msg = "A vote has been started to goto " + game + "! Vote now with \"yea\" to LEAVE and \"nay\" to STAY! Voting ends in 20 seconds!"
		send_message(msg, "all")
		sio.emit("voteStarted")
		self.voting = True

		voteTimer = Timer(20.0, self.end_switch_goto_vote, (game, delay))
		voteTimer.start()
	...
	def handleChat(self, username, message, source="twitch", uniqueID=None):
		...
		if len(commands) == 1:
			...
			if (cmd == "!discord"):
				msg = "Discord invite link: https://discord.io/rgio/"
		...
		if len(commands) == 2:
			...
			# 	self.currentSwitchGame = commands[1]
		...
		# incase they try something like "!goto sonic mania" instead of "!goto sonicmania"
		if (len(commands) >= 2):
			...
			if (commands[0] == "!goto" or (commands[0] == "!forcegoto" and username in modlist) or (commands[0] == "!setgame" and username in modlist)):
				...
				if ((game not in switchGameList2) or len(commands) > 2):
					msg = "game not found in the game list! (the name has to be exactly as it appears in the game list), use !games for a list of games."
					send_message(msg, "all")
					return

				if (game == "smo"):
					gameName, waitTime = "SUPER MARIO ODYSSEY", 30
				if (game == "botw"):
					gameName, waitTime = "The Legend of Zelda: Breath of the Wild", 20
				if (game == "celeste"):
					gameName, waitTime = "Celeste", 10
				if (game == "kirby"):
					gameName, waitTime = "Kirby Star Allies", 10
				if (game == "splatoon2"):
					gameName, waitTime = "Splatoon 2", 10
				if (game == "sonic"):
					gameName, waitTime = "Sonic Mania", 10
				if (game == "mk8"):
					gameName, waitTime = "Mario Kart 8 Deluxe", 10
				if (game == "arms"):
					gameName, waitTime = "Arms", 10
				if (game == "skyrim"):
					gameName, waitTime = "The Elder Scrolls V: Skyrim", 40
				if (game == "rocketleague"):
					gameName, waitTime = "Rocket League", 10
				if (game == "wizard"):
					gameName, waitTime = "Wizard of Legend", 10
				# if (game == "torquel"):
				# 	gameName, waitTime = "TorqueL", 10
				if (game == "fortnite"):
					gameName, waitTime = "Fortnite", 10
				if (game == "jackbox3"):
					gameName, waitTime = "The Jackbox Party Pack 3", 10
				if (game == "jackbox4"):
					gameName, waitTime = "The Jackbox Party Pack 4", 10
				if (game == "jackbox5"):
					gameName, waitTime = "The Jackbox Party Pack 5", 10
				if (game == "shovelknight"):
					gameName, waitTime = "Shovel Knight: Treasure Trove", 10
				if (game == "explosion"):
					gameName, waitTime = "Graceful Explosion Machine", 10
				if (game == "human"):
					gameName, waitTime = "Human: Fall Flat", 10
				if (game == "snipperclips"):
					gameName, waitTime = "Snipperclips", 10
				if (game == "paladins"):
					gameName, waitTime = "Paladins", 10
				if (game == "hollow"):
					gameName, waitTime = "Hollow Knight", 10
				if (game == "mta"):
					gameName, waitTime = "Mario Tennis Aces", 10
				if (game == "nes"):
					gameName, waitTime = "Nintendo Entertainment System - Nintendo Switch Online", 1
				if (game == "brawlhalla"):
					gameName, waitTime = "Brawlhalla", 10
				if (game == "undertale"):
					gameName, waitTime = "Undertale", 10
				if (game == "smash"):
					gameName, waitTime = "Super Smash Bros. Ultimate", 1
				if (game == "katamari"):
					gameName, waitTime = "Katamari Damacy REROLL", 1
				if (game == "nsmbud"):
					gameName, waitTime = "New Super Mario Bros. U Deluxe", 1
				if (game == "deltarune"):
					gameName, waitTime = "DELTARUNE Chapter 1", 1
				if (game == "tetris"):
					gameName, waitTime = "TETRIS 99", 1
				if (game == "baba"):
					gameName, waitTime = "Baba Is You", 1
				if (game == "smm2"):
					gameName, waitTime = "SUPER MARIO MAKER 2", 1
				if (game == "smp"):
					gameName, waitTime = "Super Mario Party", 1
				# if (game == "youtube"):
				# 	gameName, waitTime = "YouTube", 10
				# if (cmd == "!goto cave"):
				# 	gameName, waitTime = "Cave Story", 10
				# if (cmd == "!goto isaac"):
				# 	gameName, waitTime = "The Binding of Isaac: Afterbirth", 10
				# if (cmd == "!goto mario"):
				# 	gameName, waitTime = "Twitch Plays", 10
				# if (game == "pokemonquest"):
					# gameName, waitTime = "Pokemon Quest", 10
				# if (game == "fallout"):
					# gameName, waitTime = "Fallout Shelter", 10

				if (self.currentSwitchGame == gameName):
					msg = "We're already playing this game!"
					send_message(msg, source)
					return

				if (commands[0] == "!setgame"):
					self.currentSwitchGame = gameName
					msg = "!game " + gameName
					send_message(msg, "all")
					return

				if (commands[0] == "!forcegoto"):
					goto = self.goto_switch_game
				else:
					goto = self.goto_switch_game_vote

				goto(gameName, waitTime)
...
def emergency_lock():
	...
	# botw game over:
	# if (client.currentSwitchGame == "The Legend of Zelda: Breath of the Wild"):
